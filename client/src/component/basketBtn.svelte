<script lang="ts">
	...
	export let while_has_item = false;

	basket.useLocalStorage();
	
	console.log('basket btn added');
</script>

{#if !basket_open && (while_has_item === false || Object.keys($basket).length > 0)}
	<div on:click|stopPropagation="{() => {basket_open = true}}" class="cursor-pointer fixed bottom-10 right-10 text-center w-16 h-16 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full">
		<div class="w-6 h-6 text-base text-center rounded-full text-white bg-red-500 relative top-[-5px] right-[-40px]">
			<span class="p-1">
				{Object.keys($basket).length}
			</span>
		</div>

		<svg class="svg-icon relative top-[-20px] right-[-7px] fill-white" width="50" height="50" viewBox="0 0 20 20">
